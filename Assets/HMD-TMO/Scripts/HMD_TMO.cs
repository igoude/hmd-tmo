...
public class HMD_TMO : MonoBehaviour {
    ...
    public bool m_debugCurve = false;


    // Use this for initialization
    void Start() {
        // Update skybox
        m_panoramaMaterial.SetTexture("_Tex", m_hdrCubemap);
        ...
        m_rtPanorama = new RenderTexture(m_hdrPanorama.width, m_hdrPanorama.height, 0);
        m_rtPanorama.format = RenderTextureFormat.DefaultHDR;
        RenderTexture.active = m_rtPanorama;
        Graphics.Blit(m_hdrPanorama, m_rtPanorama);
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination) {
        ...
        // Eye adaptation
        m_keyValuesVector[0] = Mathf.Lerp(m_keyValuesVector[0], blackLum, Time.deltaTime * m_adaptedSpeed);
        m_keyValuesVector[1] = Mathf.Lerp(m_keyValuesVector[1], whiteLum, Time.deltaTime * m_adaptedSpeed);
        m_keyValuesVector[2] = Mathf.Lerp(m_keyValuesVector[2], keyValue, Time.deltaTime * m_adaptedSpeed);
        ...
        m_HmdTmoMaterial.SetFloat("_Switch", m_switch);
        m_HmdTmoMaterial.SetInt("_Debug", m_debugCurve ? 1 : 0);
